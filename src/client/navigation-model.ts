/** Full-session Turn outline enriched by the loaded window's visible segments. */
...
export interface PianoItem {
  ...
  /** Null means the official Navigator must materialize the Turn first. */
...
/** The outline is a whole-value publication: never retain entries from an older snapshot. */
export function mergeNavigationTurns(loaded: readonly TurnNavigationItem[], outline: unknown): NavigationTurn[] {
  ...
  if (Array.isArray(outline)) {
    for (const raw of outline) {
      ...
      turns.set(entry.turn, {
        turn: entry.turn, prompt: preview(entry.prompt), response: preview(entry.response),
  ...
  for (const item of loaded) {
    ...
    turns.set(item.turn, {
      turn: item.turn,
      prompt: preview(item.prompt) || old?.prompt || '',
      response: preview(item.response) || old?.response || '',
...
/**
 * One stable turn:<n> entry always survives materialization. Only known,
 * loaded segments are expanded; an outline never invents internal paragraphs.
 */
export function buildPianoItems(
  turns: readonly NavigationTurn[],
  ...
  return turns.flatMap<PianoItem>(turn => {
    ...
    return children.map((segment, index) => ({
      ...
      turn: turn.turn, title: segment.title, preview: segment.preview,
      role: segment.role, anchorKey: segment.anchorKey,
    }))
  })
}
