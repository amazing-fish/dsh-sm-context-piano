...
test('final result is independent of adjacent process prose and targets its own row', () => {
  const process = assistant('p', 1, 1, 'Working on it')
  const final = assistant('f', 1, 2, 'The final result')
...
test('streaming output promotes in place only once the authoritative tail arrives', () => {
  const running = assistant('f', 1, 2, 'Answer', { status: 'running' })
  delete running.data.finalNode
  const before = buildNavigationNodes([running])
  const final = assistant('f', 1, 2, 'Answer')
  const after = buildNavigationNodes([final, tail(final)])
  assert.equal(before[0].kind, 'output'); assert.equal(after[0].kind, 'final')
...
test('message identity mismatch and missing closing message never mark the previous one final', () => {
  const process = assistant('p', 1, 1, 'process'); const final = assistant('f', 1, 2, 'final')
...
test('one final per closed Turn, stable across history prepend', () => {
  const a = assistant('a', 1, 1, 'one'); const b = assistant('b', 2, 1, 'two')
...
test('single-select custom overrides selections; multi-select custom supplements them', () => {
  const question = { id: 'q', question: 'Pick?' }
  const answer = { id: 'q', selected: ['A'], custom: 'Custom' }
...
test('question key is stable across answer arrival; duplicate ids do not cross-pair', () => {
  const before = buildNavigationNodes([ask()])
  const after = buildNavigationNodes([ask('call1', undefined, [{ id: 'q1', selected: ['A'] }])])
...
test('error, invalid JSON and unrelated tools cannot fabricate accepted answers', () => {
  ...
  const malformed = ask(); malformed.data.root.argsRaw = '{invalid'
  assert.deepEqual(buildNavigationNodes([malformed]), [])
  const other = ask(); other.data.root.name = 'read'
  assert.deepEqual(buildNavigationNodes([other]), [])
})
test('skipped recorded answer differs from pending and unknown ids are ignored', () => {
  const keys = buildNavigationNodes([ask('call1', undefined, [{ id: 'wrong', selected: ['B'] }, { id: 'q1', selected: [] }])])
  assert.equal(keys.length, 2); assert.equal(keys[1].interactionState, 'answered')
  assert.ok(!keys[1].preview.includes('B'))
})
test('nested calls use their own stable native anchor and do not cross-call ids', () => {
  const a = ask('outer'), b = ask('inner')
  a.data.root.subCalls.push(b.data.root)
...
test('hidden node material never appears as final or interactive preview', () => {
  const question = ask(); question.visibility = 'hidden'
  assert.deepEqual(buildNavigationNodes([question]), [])
...
test('bounded untrusted JSON and cyclic call trees cannot freeze projection', () => {
  const huge = ask(); huge.data.root.argsRaw = ' '.repeat(262145)
  assert.deepEqual(buildNavigationNodes([huge]), [])
  const cycle = ask(); cycle.data.root.subCalls.push(cycle.data.root)
  assert.equal(buildNavigationNodes([cycle]).length, 1)
})
test('semantic role/kind survives the full-history model and is localized', () => {
  ...
  assert.match(semanticLabel(items.at(-1), 'zh'), /最终结果/)
  assert.match(semanticLabel(items.at(-1), 'en'), /Final result/)
